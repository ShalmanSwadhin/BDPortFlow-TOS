...
def test_create_and_delete_user(login_page, dashboard_page, user_page, test_data, env):
    ...
    user = dict(test_data['users']['new_user'])
    user['email'] = f"test.user.{uuid.uuid4().hex[:8]}@bdport.gov.bd"
    ...
    assert user_page.is_user_visible(user['name'], user['email']), 'Newly created user should appear in the list.'

    user_page.delete_user(user['name'], user['email'])
    user_page.confirm_deletion()

    user_page.search_user(user['email'])
    assert not user_page.is_user_visible(user['name'], user['email']), 'Deleted user should no longer be visible.'
